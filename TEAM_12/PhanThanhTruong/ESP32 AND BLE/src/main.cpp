...
#include <math.h> // Thêm thư viện toán học để tính độ lệch khoảng cách
...
// Các biến mới phục vụ tính năng Live Tracking (Cập nhật di chuyển)
float lastReportedDistance = -1.0;     // Lưu lại vị trí lần báo cáo cuối cùng
unsigned long lastMessageTime = 0;     // Thời điểm gửi tin nhắn cuối cùng
const unsigned long MSG_COOLDOWN = 8000; // Phải đợi 8 giây mới được nhắn tiếp (Chống spam Telegram)
const float MOVEMENT_SENSITIVITY = 0.5;  // Phải di chuyển > 0.5m thì mới báo cáo
...
void loop() {
  ...
  // LOGIC HIỆN DIỆN & LIVE TRACKING
  if (distance <= DISTANCE_THRESHOLD) {
    lastPresenceTime = millis();
    
    // Kịch bản 1: Vừa mới bước vào phòng
    if (!isRoomActive) {
      isRoomActive = true;
      ...
      // Kịch bản 2: Đã ở trong phòng và đang di chuyển (Live Tracking)
      // Kiểm tra xem khoảng cách thay đổi > 0.5m VÀ đã hết 8 giây chống spam chưa
    ...
    // Kịch bản 3: Rời khỏi phòng (Vượt quá 3m)
    if (isRoomActive && (millis() - lastPresenceTime > TIMEOUT)) {
      isRoomActive = false;
      lastReportedDistance = -1.0; // Reset lại lịch sử vị trí
