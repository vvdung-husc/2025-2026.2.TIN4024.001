...
Dự án được mô phỏng hoàn toàn trên [Wokwi](https://wokwi.com) và xây dựng bằng **PlatformIO** + **Arduino Framework**.
...
| *(tự động)* | Cảnh báo khi PIR phát hiện chuyển động |
...
|---|---|
| GPIO 23 | Anode LED (qua R 220Ω) |
| GPIO 27 | OUTPUT của cảm biến PIR |
...
> **Lưu ý:** File `secrets.h` chứa `BOT_TOKEN` và `CHAT_ID` **không** được đưa lên repository (đã có trong `.gitignore`). Xem hướng dẫn tạo file này ở phần [Cài đặt](#-cài-đặt--cấu-hình).
...
- Cài extension **PlatformIO IDE** từ VS Code Marketplace.
...
1. Sau khi nạp code, ESP32 sẽ kết nối WiFi (Serial Monitor hiển thị `WiFi connected`).
2. Mở ứng dụng Telegram, tìm tên bot bạn đã tạo.
3. Gửi các lệnh:
...
4. Khi cảm biến PIR phát hiện chuyển động, bot sẽ **tự động gửi cảnh báo** đến Telegram.

---

## Mô phỏng trên Wokwi

Dự án hỗ trợ mô phỏng trực tiếp trên [Wokwi](https://wokwi.com) bằng extension **Wokwi for VS Code**.

### Cách chạy mô phỏng

1. Cài extension **[Wokwi for VS Code](https://marketplace.visualstudio.com/items?itemName=wokwi.wokwi-vscode)**.
...
### Mô phỏng mạch trên Wokwi
![Wokwi Simulation](images%20result/wokwi.png)

### Kết quả điều khiển trên Telegram
![Telegram Bot](images%20result/telebot.png)
